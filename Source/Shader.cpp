...
HRESULT create_cs_from_cso(ID3D11Device* device, 
    const char* cso_name, 
    ID3D11ComputeShader** comp_shader) {
    ...
    HRESULT hr = device->CreateComputeShader(cso_data.get(), cso_sz, nullptr, comp_shader);
    _ASSERT_EXPR(SUCCEEDED(hr), HRTrace(hr));
