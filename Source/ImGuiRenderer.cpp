	...
	: hWnd(hWnd)
{
	...
	ImFont* font = io.Fonts->AddFontFromFileTTF("resources\\Textures\\ArialUni.ttf", 18.0f, NULL, io.Fonts->GetGlyphRangesJapanese());
	...
	// 頂点シェーダー
	{
		...
		create_vs_from_cso(device, "Shader\\ImGuiVS.cso", vertexShader.ReleaseAndGetAddressOf(),
			inputLayout.ReleaseAndGetAddressOf(), inputElementDesc, ARRAYSIZE(inputElementDesc));
	}

	// ピクセルシェーダー
	{
		create_ps_from_cso(device, "Shader\\ImGuiPS.cso", pixelShader.GetAddressOf());
	}
