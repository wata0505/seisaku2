...
Framebuffer::Framebuffer(ID3D11Device* device, uint32_t width, uint32_t height) {
   ...
   // Microsoft::WRL::ComPtr<ID3D11Texture2D> renderTargetBuffer;
    ...
    //light = std::make_shared<Light>(device);
    Light* light = new Light({ 1,-1,0 },0, 2.0, 2.0, 2.0);
    LightManager::Instance().Register(light);
    light = new Light({ -1,-1,0 }, 0, 2.5, 2.5, 2.5);
    LightManager::Instance().Register(light);
    light = new Light({ 1,1,1 },0, 0.5, 0.5, 0.5);
    LightManager::Instance().Register(light);
...
void Framebuffer::EffectActivate(ID3D11DeviceContext* immediate_context)
{
    ...
    // レンダーターゲットビュー設定
    immediate_context->OMSetRenderTargets(
        1, renderTargetView[0].GetAddressOf(), depthStencilView.Get());
}
...
void Framebuffer::Lighting(ID3D11DeviceContext* immediate_context) {
    ...
    //ターゲット→ライトバッファ
    immediate_context->OMSetRenderTargets(
        1, renderTargetView[4].GetAddressOf(), depthStencilView.Get());
    ...
    immediate_context->OMSetRenderTargets(
        1, renderTargetView[5].GetAddressOf(), depthStencilView.Get());
