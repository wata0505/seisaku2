...
void GamePad::Update()
{
	...
	if (XInputGetState(slot, &xinputState) == ERROR_SUCCESS)
	{
		...
		//
		if (pad.wButtons & XINPUT_GAMEPAD_A) newButtonState |= BTN_PAD_A;
	...
	// キーボードでエミュレーション
	{
		...
		if (GetAsyncKeyState('G') & 0x8000) newButtonState |= BTN_RIGHT_SHOULDER;
		...
		if (GetAsyncKeyState('T') & 0x8000) newButtonState |= BTN_RIGHT_TRIGGER;
		if (GetAsyncKeyState('Y') & 0x8000) newButtonState |= BTN_LEFT_TRIGGER;
